...
$stmt = $conexao->prepare("SELECT COUNT(*) FROM quimicos WHERE numero_unico = :numero_unico");
...
function gerarPDF($dados) {
    ...
    $pdf->Cell(0, 10, utf8_decode('Anexo "G"'), 0, 1, 'C');
    $pdf->Cell(0, 10, utf8_decode('Modelo de Termo de Fiscalização/Vstoria de empresas que realizam '), 0, 1, 'C');
    $pdf->Cell(0, 10, utf8_decode('atividades com produtos quimicos'), 0, 1, 'C');
    $pdf->Ln(10);
    ...
    $pdf->Cell(0, 10, utf8_decode('TERMO DE FISCALIZAÇÃO/VISTORIA DE EMPRESAS QUE REALIZAM '), 0, 1, 'C');
    $pdf->Cell(0, 10, utf8_decode('ATIVIDADES COM PRODUTOS QUIMICOS'), 0, 1, 'C');
 ...
 // Texto fixo "Número"
    ...
    $pdf->Cell(0, 5, utf8_decode('1.IDENTIFICAÇÃO FISCALIZADO '), 0, 1, 'L');
    ...
    $pdf->SetFont('Arial', '', 12);
    $pdf->Cell(50, 10, utf8_decode($dados['telefone']), 'TBL', 0, 'L'); // Apenas borda superior, esquerda e inferior

    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(40, 10, utf8_decode('Tel Residencial:'), 1, 0, 'L');
    $pdf->SetFont('Arial', '', 12);
    $pdf->MultiCell(60, 10, utf8_decode($dados['telefoneResidencial']), 1, 'L');
    $pdf->Ln(2); // Pequeno espaçamento
   ...
   // Definir título das colunas (N Ordem | Itens a Verificar | SIM | Não | Não se aplica | OBS)
$pdf->SetFont('Arial', 'B', 12);

// Cabeçalho das colunas
$pdf->Cell(20, 10, utf8_decode('N Ordem'), 1, 0, 'C'); // N Ordem 
$pdf->Cell(60, 10, utf8_decode('Itens a Verificar'), 1, 0, 'C'); // Itens a Verificar
$pdf->Cell(20, 10, utf8_decode('Sim'), 1, 0, 'C'); // SIM
$pdf->Cell(20, 10, utf8_decode('Não'), 1, 0, 'C'); // Não
$pdf->Cell(30, 10, utf8_decode('Não se aplica'), 1, 0, 'C'); // Não se aplica
$pdf->Cell(40, 10, utf8_decode('Observações'), 1, 1, 'C'); // Observações
...
foreach ($dados['respostas'] as $i => $resposta) {
    ...
    // Se resposta estiver vazia, exibe "(Vazio)"
    $respostaDebug = !empty($resposta) ? $resposta : "(Vazio)";

    // Ajusta altura conforme o texto
    $alturaPergunta = $pdf->GetStringWidth($pergunta) > 60 ? 20 : 10;
    ...
    $x = $pdf->GetX();
    $y = $pdf->GetY();
    $pdf->MultiCell(60, 5, utf8_decode($pergunta), 0, 'L');
    $pdf->Rect($x, $y, 60, $alturaLinha);
 ...
 $pdf->Ln(10); // Espaçamento
    ...
    //Identificação do fiscalizado    
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(0, 5, utf8_decode('a.Operação com produtos químicos controlados'), 0, 1, 'L');
    $pdf->Ln(5); // Espaçamento

    $perguntasFixasExecucaoA = [
 ...
 foreach ($dados['respostas'] as $i => $respostaExecucao) {
    $pergunta = $perguntasFixasExecucaoA[$i] ?? 'Pergunta não definida';
    $observacao = $dados['observacoesExecucao'][$i] ?? 'Sem observação';

    // Se não existir a pergunta, pula
    ...
    // Respostas (SIM / NÃO / Não se aplica)
    $pdf->Cell(20, $alturaLinha, utf8_decode($respostaExecucao == 'Sim' ? 'X' : ''), 1, 0, 'C');
    $pdf->Cell(20, $alturaLinha, utf8_decode($respostaExecucao == 'Não' ? 'X' : ''), 1, 0, 'C');
    $pdf->Cell(30, $alturaLinha, utf8_decode($respostaExecucao == 'Não se Aplica' ? 'X' : ''), 1, 0, 'C');
    ...
    $pdf->SetY($y + $alturaLinha);
     ...
     //Identificação do fiscalizado
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(0, 5, utf8_decode('b.Prevenção e combate a incêndio'), 0, 1, 'L');
    $pdf->Ln(5); // Espaçamento

    $perguntasFixasExecucaoB = [
      "Há extintores adequados em todas as áreas (administrativa, de fabricação e depósitos)",
        /*"Há hidrantes e mangueiras dispostas estrategicamente nas áreas de fabricação e depósitos?",
    ...
    foreach ($dados['respostaExecucao'] as $i => $respostaExecucao) {
          $pergunta = $perguntasFixasExecucaoB[$i] ?? 'Pergunta não definida';
          $observacao  = $dados['observacoesExecucao'][$i] ?? 'Sem observação';
        ...
        $alturaPergunta = $pdf->GetStringWidth($pergunta) > 60 ? 40 : 10; // Aumenta a altura se necessário
        $alturaObservacao = $pdf->GetStringWidth($observacao) > 40 ? 20 : 10;
        $alturaLinha = max($alturaPergunta, $alturaObservacao);
...
$pdf->Cell(30, $alturaLinha, utf8_decode($respostaExecucao == 'Não se aplica' ? 'X' : ''), 1, 0, 'C');
    ...
    // Identificação do fiscalizado
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 5, utf8_decode('c. Higiene e segurança do trabalho'), 0, 1, 'L');
$pdf->Ln(5); // Espaçamento

$perguntasFixasExecucaoC = [
    ...
    "Há algum tipo de alcalinizante, como barrilha ou cal para neutralização de vazamento de HF?",
...
foreach ($dados['respostaExecucao'] as $i => $respostaExecucao) {
    $pergunta = $perguntasFixasExecucaoC[$i] ?? 'Pergunta não definida';
    $observacao = $dados['observacoesExecucao'][$i] ?? 'Sem observação';
    ...
    if ($pergunta !== "Pergunta não definida") {
        ...
        // Vai direto para a próxima linha
        $pdf->SetY($y + $alturaLinha);
    }
}
     ...
     //Identificação do fiscalizado
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(0, 5, utf8_decode('d.Segurança de área'), 0, 1, 'L');
    $pdf->Ln(5); // Espaçamento

    $perguntasFixasExecucaoD = [
      ...
      foreach ($dados['respostaExecucao'] as $i => $respostaExecucao) {
    $pergunta = $perguntasFixasExecucaoD[$i] ?? 'Pergunta não definida';
    $observacao = $dados['observacoesExecucao'][$i] ?? 'Sem observação';
...
foreach ($camposInfracao as $rotulo => $valor) {
   ...
   $pdf->Ln(5);
    ...
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(0, 10, utf8_decode('4.Distâncias de segurança'), 0, 1, 'L');

            $pdf->Cell(40, 10, utf8_decode('nr_de_ord'), 1, 0, 'C');
            $pdf->Cell(35, 10, utf8_decode('codigo_pav_dep'), 1, 0, 'C');
            $pdf->Cell(35, 10, utf8_decode('destinacao'), 1, 0, 'C');
            $pdf->Cell(35, 10, utf8_decode('barricado'), 1, 0, 'C');
            $pdf->Cell(45, 10, utf8_decode('dotacao'), 1, 1, 'C');

            foreach ($dados['nr_de_ord'] as $i => $nr_de_ord) {
            $codigo_pav_dep = $dados['codigo_pav_dep'][$i] ?? 'Sem observação';
            $destinacao = $dados['destinacao'][$i] ?? 'Sem observação';
            $barricado = $dados['barricado'][$i] ?? 'Sem observação';
            $dotacao = $dados['dotacao'][$i] ?? 'Sem observação';

           
            
            
            $pdf->Cell(35, 10, utf8_decode($nr_de_ord), 1, 0, 'C');
            $pdf->Cell(50, 10, utf8_decode($codigo_pav_dep), 1, 0, 'C');
            $pdf->Cell(35, 10, utf8_decode($destinacao), 1, 0, 'C');
            $pdf->Cell(35, 10, utf8_decode($barricado), 1, 0, 'C');
            $pdf->Cell(35, 10, utf8_decode($dotacao), 1, 1, 'C');
        }
        ...
        $pdf->Cell(40, 10, utf8_decode('edificio_habitado'), 1, 0, 'C');
        $pdf->Cell(35, 10, utf8_decode('ferrovia'), 1, 0, 'C');
        $pdf->Cell(35, 10, utf8_decode('rodovia'), 1, 0, 'C');
        $pdf->Cell(35, 10, utf8_decode('predio_proximo'), 1, 0, 'C');
        $pdf->Cell(45, 10, utf8_decode('cod_predio_proximo'), 1, 1, 'C');

        foreach($dados['edificio_habitado'] as $i => $edificio_habitado){
            $ferrovia = $dados['ferrovia'][$i] ?? 'Sem observação';
            $rodovia = $dados['rodovia'][$i] ?? 'Sem observação';
            $predio_proximo = $dados['predio_proximo'][$i] ?? 'Sem observação';
            $cod_predio_proximo = $dados['cod_predio_proximo'][$i] ?? 'Sem observação';
...
/*
    // Outras informações
    $pdf->Cell(95, 10, utf8_decode('Lista de Deficiências: ' . $dados['lista_deficiencia']), 0, 0);
    $pdf->Cell(95, 10, utf8_decode('Observações Gerais: ' . $dados['observacoes_gerais']), 0, 1);

    foreach ($dados['infracao'] as $i => $infracao) {
        $pdf->Cell(95, 10, utf8_decode('Infração: ' . $infracao), 0, 0);
        $pdf->Ln();
    }

    // Informações sobre infrações
    $pdf->Cell(95, 10, utf8_decode('Qtd Autos de Infração: ' . $dados['qtd_autos_infracao']), 0, 1);
    $pdf->Cell(95, 10, utf8_decode('Qtd Termos de Infração: ' . $dados['qtd_termos_aprensao']), 0, 0);
    $pdf->Cell(95, 10, utf8_decode('Qtd Termos Depositário: ' . $dados['qtd_termos_depositario']), 0,1);
    $pdf->Cell(95, 10, utf8_decode('Especificar Deficiências: ' . $dados['especificar_deficiencias']), 0,0);
    $pdf->Cell(95, 10, utf8_decode('Prazo Deficiências: ' . $dados['prazo_deficiencias']), 0,1);
    $pdf->Cell(95, 10, utf8_decode('Nome Fiscal Militar: ' . $dados['nome_fiscal_militar']), 0,0);
    $pdf->Cell(95, 10, utf8_decode('P/G Fiscal Militar: ' . $dados['fiscal_pg']), 0,1);
    $pdf->Cell(95, 10, utf8_decode('Idt. Fiscal Militar: ' . $dados['idtmilitar']), 0,0);
    $pdf->Cell(95, 10, utf8_decode('OM Fiscal Militar: ' . $dados['ommilitar']), 0,1);
    $pdf->Cell(95, 10, utf8_decode('Empresa Fiscalizada: ' . $dados['nome_empresa']), 0,0);
    $pdf->Cell(95, 10, utf8_decode('CPF Empresa: ' . $dados['cpf_empresa']), 0,1);
    $pdf->Cell(95, 10, utf8_decode('Testemunha 1: ' . $dados['testemunha1']), 0,0);
    $pdf->Cell(95, 10, utf8_decode('Idt Testemunha 1: ' . $dados['itdtestemunha1']), 0,1);
    $pdf->Cell(95, 10, utf8_decode('Testemunha 2: ' . $dados['testemunha2']), 0,0);
...
foreach ($assinaturas as $i => $assinatura) {
 ...
 $nomeArquivo = 'quimicos_' . $dados['user_id'] . '_' . $timestamp . '.pdf'; // Adicionando o timestamp

 // Saída do PDF
 $pdf->Output('F', 'PDF/FormulariosPDF/' . $nomeArquivo); // Salva o PDF na pasta PDF
...
try {
    ...
    $nr_de_ord = json_decode($_POST['nr_de_ord'], true);
    $codigo_pav_dep = json_decode($_POST['codigo_pav_dep'], true);
    $destinacao = json_decode($_POST['destinacao'], true);
    $barricado = json_decode($_POST['barricado'], true);
    $dotacao = json_decode($_POST['dotacao'], true);
    $edificio_habitado = json_decode($_POST['edificio_habitado'], true);
    $ferrovia = json_decode($_POST['ferrovia'], true);
    $rodovia = json_decode($_POST['rodovia'], true);
    $predio_proximo = json_decode($_POST['predio_proximo'], true);
    $cod_predio_proximo = json_decode($_POST['cod_predio_proximo'], true);
    ...
    $respostaExecucao = json_decode($_POST['respostaExecucao'], true);
    $observacoesExecucao = json_decode($_POST['observacoesExecucao'], true);
   ...
   /* // Verificar campos
    ...
    substituirPorNaoSeAplica($observacoesExecucao);
    substituirPorNaoSeAplica($respostas);
    substituirPorNaoSeAplica($respostaExecucao);
    ...
    if (is_array($respostaExecucao)) {
        verificarPreenchimento($respostaExecucao, 'Verificação das Condições de Execução (Sim) ou (Não)');
    } else {
        $naoPreenchidas[] = "Verificação das Condições de Execução (Sim) ou (Não) não foi preenchida.";
    }

    if (is_array($observacoesExecucao)) {
        verificarPreenchimento($observacoesExecucao, 'Verificação das Condições de Execução e Observações');
    } else {
    ...
    */
    ...
    if(strlen($telefone) < $maximoComprimentoCamposTelefone ){
        $response = array("status" => "error", "message" => "O Campo do telefone não pode conter menos que $maximoComprimentoCamposTelefone caracteres");
        echo json_encode($response);
        exit;
    }
    //Verificar tamanhos dos campos
    ...
    if(strlen($telefoneResidencial) < $maximoComprimentoCamposTelefoneResi  ){
        $response = array("status" => "error", "message" => "O Campo do telefone residencial não pode conter menos que $maximoComprimentoCamposTelefoneResi caracteres");
        echo json_encode($response);
        exit;
    }    
    ...
    if(strlen($data) > $maximoComprimentoCampoData ){
        $response = array("status" => "error", "message" => "A Data não pode conter mais que $maximoComprimentoCampoData caracteres");
        echo json_encode($response);
        exit;
    }
    ...
    $maximoComprimentoCampoDestinacao = 100;
    ...
    $maximoComprimentoCampoDotacao = 100;
    ...
    $maximoComprimentoCampoEdificiohabitado = 30;
    if(strlen($edificio_habitado) > $maximoComprimentoCampoEdificiohabitado ){
        $response = array("status" => "error", "message" => "O Ferrovia não pode conter mais que $maximoComprimentoCampoEdificiohabitado caracteres");
        echo json_encode($response);
        exit;
    }
    $maximoComprimentoCampoFerrovi = 30;
    ...
    $maximoComprimentoCampoRodovia = 30;
    ...
    $maximoComprimentoCampoPrediomaisproximo = 30;
 ...
 $maximoComprimentoCampoCodprediomaisproximo = 30;
    ...
    */
    ...
    $resultadoImagens = base64paraImagens($imagens_base64); 
        ...
        //Recuperar o último numero apos o insert
        $numeroUnicoInseriodo = $conexao-> lastInsertId();
    ...
    $dadosPDF = [
        ...
        'nr_de_ord' => $nr_de_ord,
        'codigo_pav_dep' => $codigo_pav_dep,
        'destinacao' => $destinacao,
        'barricado' => $barricado,
        'dotacao' => $dotacao,
        'edificio_habitado' => $edificio_habitado,
        'ferrovia' => $ferrovia,
        'rodovia' => $rodovia,
        'predio_proximo' => $predio_proximo,
        'cod_predio_proximo' => $cod_predio_proximo,
    ...
    // Inserir no banco
$stmt = $conexao->prepare("INSERT INTO quimicos (user_id, razaoSocial, email, data, trcr, endereco, telefone, telefoneResidencial, cnpj, pdf_file) VALUES (:user_id, :razaoSocial, :email, :data, :trcr, :endereco, :telefone, :telefoneResidencial, :cnpj, :pdf_file)");
...
$quimicos_id = $conexao->lastInsertId();

$stmtRespostas = $conexao->prepare("INSERT INTO respostas_empresa_quimicos (quimicos_id, user_id, respostaEmpresa, observacoes) VALUES (:quimicos_id, :user_id, :respostaEmpresa, :observacoes)");
for ($i = 0; $i < count($respostas); $i++) {
    $stmtRespostas->bindParam(':user_id', $user_id);
    $stmtRespostas->bindParam(':quimicos_id', $quimicos_id);
...
$stmt_seguranca = $conexao->prepare("INSERT INTO distancias_seguranca_quimicos (user_id, quimicos_id, nr_de_ord, codigo_pav_dep, destinacao, barricado, dotacao, edificio_habitado, ferrovia, rodovia,predio_proximo,cod_predio_proximo  ) VALUES (:user_id, :quimicos_id, :nr_de_ord, :codigo_pav_dep, :destinacao, :barricado, :dotacao, :edificio_habitado, :ferrovia, :rodovia, :predio_proximo , :cod_predio_proximo)");
for ($i = 0; $i < count($nr_de_ord); $i++) {
$stmt_seguranca->bindParam(':user_id', $user_id);
$stmt_seguranca->bindParam(':quimicos_id', $quimicos_id);
$stmt_seguranca->bindValue(':nr_de_ord', $nr_de_ord[$i]);
$stmt_seguranca->bindValue(':codigo_pav_dep', $codigo_pav_dep[$i]);
$stmt_seguranca->bindValue(':destinacao', $destinacao[$i]);
$stmt_seguranca->bindValue(':barricado', $barricado[$i]);
$stmt_seguranca->bindValue(':dotacao', $dotacao[$i]);
$stmt_seguranca->bindValue(':edificio_habitado', $edificio_habitado[$i]);
$stmt_seguranca->bindValue(':ferrovia', $ferrovia[$i]);
$stmt_seguranca->bindValue(':rodovia', $rodovia[$i]);
$stmt_seguranca->bindValue(':predio_proximo', $predio_proximo[$i]);
$stmt_seguranca->bindValue(':cod_predio_proximo', $cod_predio_proximo[$i]);
$stmt_seguranca->execute();
}


// Inserção na tabela respostas_execucao_quimicos
$stmt_respostas_execucao = $conexao->prepare("INSERT INTO respostas_execucao_quimicos(user_id, quimicos_id, respostaExecucao, observacoesExecucao) VALUES (:user_id, :quimicos_id, :respostaExecucao, :observacoesExecucao)");
for ($i = 0; $i < count($respostaExecucao); $i++) {
    $stmt_respostas_execucao->bindParam(':user_id', $user_id);
    $stmt_respostas_execucao->bindParam(':quimicos_id', $quimicos_id);
    $stmt_respostas_execucao->bindValue(':respostaExecucao', $respostaExecucao[$i]);
    $stmt_respostas_execucao->bindValue(':observacoesExecucao', $observacoesExecucao[$i]);
    $stmt_respostas_execucao->execute();
}

// Inserção na tabela deficiencias_observacoes_quimicos
$smtpinfracao = $conexao->prepare("INSERT INTO deficiencias_observacoes_quimicos (user_id, quimicos_id, lista_deficiencia, observacoes_gerais, infracao, qtd_autos_infracao, qtd_termos_aprensao, qtd_termos_depositario, especificar_deficiencias_encontradas, prazo_deficiencias, nome_fiscal_militar, fiscal_pg, idtmilitar, ommilitar, nome_empresa, cpf_empresa, testemunha1, itdtestemunha1, testemunha2, itdtestemunha2) VALUES (:user_id, :quimicos_id, :lista_deficiencia, :observacoes_gerais, :infracao, :qtd_autos_infracao, :qtd_termos_aprensao, :qtd_termos_depositario, :especificar_deficiencias_encontradas, :prazo_deficiencias, :nome_fiscal_militar, :fiscal_pg, :idtmilitar, :ommilitar, :nome_empresa, :cpf_empresa, :testemunha1, :itdtestemunha1, :testemunha2, :itdtestemunha2)");

// Bind dos parâmetros fora do loop
$smtpinfracao->bindParam(':user_id', $user_id);
$smtpinfracao->bindParam(':quimicos_id', $quimicos_id);
...
if (!is_array($listaApreensao) || empty($listaApreensao)) {
    ...
    $stmt = $conexao->prepare("INSERT INTO quimicos_apreensao (quimicos_id, data_hora, estadoIdSelecionado, cidade, produto, qtdApreensao, unidade, tipo, marca, obs) 
                               VALUES (:quimicos_id, :data_hora, :estadoIdSelecionado, :cidade, :produto, :qtdApreensao, :unidade, :tipo, :marca, :obs)");
            ...
            if (!empty($apreensao['produtos']) && is_array($apreensao['produtos'])) {
                foreach ($apreensao['produtos'] as $produto) {
                    ...
                    if (!empty($produtoNome)) {
                        $stmt->bindParam(':quimicos_id', $quimicos_id, PDO::PARAM_INT);
...
if (!is_array($listaInfracao) || empty($listaInfracao)) {
    ...
    // Preparar a query de inserção
    $stmt = $conexao->prepare("INSERT INTO quimicos_infracao 
        (quimicos_id, data_hora_infracao, estadoIdSelecionado_infracao, cidade_infracao, 
...
if (!is_array($listaDepositario) || empty($listaDepositario)) {
    ...
    // Preparar a query de inserção
    $stmt = $conexao->prepare("INSERT INTO quimicos_fiel_depositario 
        (quimicos_id, data_hora_depositario, estadoIdSelecionado_depositario, cidade_depositario, 
...
try {
    ...
    // Corpo do e-mail em HTML com as variáveis corretas
     ...
     for ($i = 0; $i < count($respostaExecucao); $i++) {
         $respostasObservacoesExecucaoHTML .= '<strong>Resposta ' . ($i + 1) . ':</strong> ' . $respostaExecucao[$i] . '<br />';
         $respostasObservacoesExecucaoHTML .= '<strong>Observação ' . ($i + 1) . ':</strong> ' . $observacoesExecucao[$i] . '<br /><br />';
     }
