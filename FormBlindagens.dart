...
class FormBlindagens extends StatefulWidget {
  ...
  FormBlindagens({required this.user_id, required this.session_token, required this.idtMilitar, required this.postoGraduacao, required this.omMilitar});

  @override
  _FormBlindagensState createState() => _FormBlindagensState();
}
class _FormBlindagensState extends State<FormBlindagens> {
  ...
  final String a = "a.Quanto às atividades e produtos";
  final String b = "b.Quanto à aplicação de blindagem balística automotiva (blindadoras)";
  final String c = "c.Quanto à aplicação de blindagem blalística em estruturas arquitetônicas";

  //blindagens_Deficiencias
  final TextEditingController lista_deficiencia = TextEditingController();

  //blindagens_Observacoes_gerais
  ...
  //blindagens_Deficiencias_Encontradas
...
// Método para enviar os dados ao servidor
  Future<void> enviarDadosFormulario(BuildContext context) async {
    ...
    String arquivoJson = "FormFiscalizacao/FormBlindagens.php";
...
// Método para salvar os dados localmente
  Future<void> salvarDadosLocalmente() async {
    // Verifica a conectividade usando connectivity_plus
    ...
    if (semConexao) {
      ...
      List<String> dadosLocaisBlindagem = prefs.getStringList(
          'dadosLocaisBlindagem') ?? [];
...
// Debug: Exibir a lista final antes do envio
    ...
    Map<String, dynamic> dados = {
      ...
      'listaApreensao': listaApreensao, // Adicionando a lista de apreensão
      'listaInfracao': listaInfracao, // Adicionando a lista de infração
      'listaDepositario': listaDepositario, // Adicionando a lista de fiel depositário

    };

    // Adiciona dados convertidos à lista de dados locais
    dadosLocaisBlindagem.add(jsonEncode(dados));
    await prefs.setStringList('dadosLocaisBlindagem', dadosLocaisBlindagem);
...
// data.clear();
      ...
      fiscal_pg.clear();
      idtMilitarController.clear();
      omMilitarController.clear();
      nome_empresa.clear();
      cpf_empresa.clear();
      testemunha1.clear();
...
// Você pode também resetar as variáveis de texto para os valores padrão
  ...
  Future<void> enviarDadosSalvosLocalmente(BuildContext context) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    List<String> dadosLocaisBlindagem = prefs.getStringList('dadosLocaisBlindagem') ?? [];

    if (dadosLocaisBlindagem.isEmpty) {
      print("Nenhum dado salvo localmente para enviar.");
      return;
    }
    List<Map<String, dynamic>> listaApreensao = [];
...
// Debug: Exibir a lista final antes do envio
    ...
    for (String dadosJson in dadosLocaisBlindagem) {
      try {
...
// Método para limpar os dados salvos localmente
  Future<void> limparDadosLocais() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.remove('dadosLocaisBlindagem'); // Limpa todos os dados salvos
  }
...
// Debug: verificar quantos dados locais existem
    print("Dados locais carregados: ${dadosLocaisBlindagem.length}");

    List<String> imagensBase64 = [];

    for (String dado in dadosLocaisBlindagem) {
      try {
...
// Método para remover registro local
  Future<void> removerRegistroLocal(String dados) async {
    ...
    dadosLocaisBlindagem.removeWhere((registro) => registro == dados);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Blindagem Balística e Veículos" ),
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        "Termo de Fiscalização/Vistoria de Empresas que Realizam Atividades com Blindagens Balisticas e Veículos Blindados",
                        style: TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.bold,
                          color: Colors.white,
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () async {
                          try {
                            ...
                            // Após o envio, remova o registro local
                  ...
                  children: [
                    Expanded(
                      child: RadioListTile<String>(
                        ...
                        value: "Sim",
                        groupValue: respostaEmpresa.length > index
                            ? respostaEmpresa[index]
                            : null,
                        onChanged: (value) {
                          setState(() {
  ...
  Widget buildFormFieldEmpresa(String hint, String initialValue, int index,
    ...
    return TextField(
      ...
      controller: TextEditingController(
          text: initialValue.isEmpty ? "S/A" : initialValue),
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () async {
                          await enviarDadosParaServidor(
                            ...
                            dadosMap['postoGraduacaoController'] ?? '',
                            dadosMap['idtmMilitarController'] ?? '', // Novo parâmetro
                            dadosMap['omMilitarController'] ?? '',
